...
import { Send, Plus, Bot, User, Trash2, PanelLeftClose, PanelLeftOpen } from "lucide-react";
...
function useAutoResize(ref, { minRows = 1, maxRows = 6 } = {}) {
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const rows = Math.min(maxRows, Math.max(minRows, Math.ceil(contentHeight / lineHeight)));
      el.rows = rows;
    };
    resize();
...
export default function Chatgpt() {
  ...
  const send = async () => {
    ...
    const thinkingMsg = { id: crypto.randomUUID(), role: "assistant", content: "Thinking…" };

    setConversations(prev => prev.map(c => c.id === activeId ? { ...c, messages: [...(c.messages || []), userMsg, thinkingMsg] } : c));
    setInput("");
    ...
    setTimeout(() => {
      setConversations(prev => prev.map(c => {
        ...
        // Use first user line to title the chat
        const title = c.title === "New chat" ? trimmed.slice(0, 28) + (trimmed.length > 28 ? "…" : "") : c.title;
  ...
  return (
    <div className="h-dvh w-screen bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100 flex overflow-hidden">
      ...
      <aside className={classNames(
        ...
        <div className={classNames("h-full", sidebarOpen ? "opacity-100" : "opacity-0 pointer-events-none", "transition-opacity duration-300")}> 
          <div className="h-16 px-4 flex items-center justify-between">
            {/* On mobile show only +, on desktop show text */}
            <button
              onClick={createChat}
          ...
          <div className="px-2 pb-4 overflow-y-auto h-[calc(100dvh-4rem)]">
            {conversations.map(c => (
              <div key={c.id} className={classNames(
                ...
                c.id === activeId ? "bg-zinc-100 dark:bg-zinc-800" : "hover:bg-zinc-100/60 dark:hover:bg-zinc-800/60"
              )}>
                <button onClick={() => setActiveId(c.id)} className="flex-1 text-left truncate text-sm">{c.title}</button>
                <button onClick={() => deleteChat(c.id)} className="opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-zinc-200 dark:hover:bg-zinc-700">
      ...
      <main className="flex-1 flex flex-col">
        ...
        <div className="h-14 shrink-0 border-b border-zinc-200 dark:border-zinc-800 bg-white/70 dark:bg-zinc-900/60 backdrop-blur supports-[backdrop-filter]:bg-white/50 supports-[backdrop-filter]:dark:bg-zinc-900/40 sticky top-0 z-10">
          <div className="h-full px-3 md:px-6 flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <button onClick={() => setSidebarOpen(o => !o)} className="rounded-xl border border-zinc-300 dark:border-zinc-700 p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 md:hidden">
                {sidebarOpen ? <PanelLeftClose className="h-5 w-5"/> : <PanelLeftOpen className="h-5 w-5"/>}
              </button>
              <span className="font-medium">ChatGPT UI</span>
            </div>
            <div className="hidden md:flex items-center gap-2">
              <button onClick={createChat} className="inline-flex items-center gap-2 rounded-2xl border border-zinc-300 dark:border-zinc-700 px-3 py-2 text-sm hover:bg-zinc-100 dark:hover:bg-zinc-800">
                <Plus className="h-4 w-4" /> New chat
              </button>
              <button onClick={() => setTheme(t => t === "dark" ? "light" : "dark")} className="rounded-xl border border-zinc-300 dark:border-zinc-700 px-2 py-2 text-sm hover:bg-zinc-100 dark:hover:bg-zinc-800">
          ...
          <div className="max-w-3xl mx-auto py-4 md:py-6">
            ...
            {active?.messages?.map(m => (
              <div key={m.id} className="w-full"> 
                <div className={classNames("grid grid-cols-[auto,1fr] gap-3 md:gap-4 py-4")}>
                  <div className={classNames(
                    "h-9 w-9 md:h-10 md:w-10 rounded-lg flex items-center justify-center",
                    m.role === "assistant" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300" : "bg-zinc-200 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300"
                  )}>
        ...
        {/* Composer (mobile-friendly, safe area) */}
